...
**parameter**       | **value**
 ...
 `v.H`, `v.P`       | [`r LHC.parameter.ranges$v.H[1]`, `r LHC.parameter.ranges$v.H[2]`], [`r LHC.parameter.ranges$v.P[1]`, `r LHC.parameter.ranges$v.P[2]`]
  `r.H`, `r.P`       | [`r LHC.parameter.ranges$r.H[1]`, `r LHC.parameter.ranges$r.H[2]`], [`r LHC.parameter.ranges$r.P[1]`, `r LHC.parameter.ranges$r.P[2]`]
 ...
 `MaxArea`   | [`r LHC.parameter.ranges$MaxArea[1]`, `r LHC.parameter.ranges$MaxArea[2]`]
...
```{r 5LHCtablehtml, eval=!params$ispdf, cache=TRUE, dependson=c('5_table-functions','5_LHC-load')}
exp.table(PARS, numberOfParameters = 15, caption = 'ACTUAL parameter values')
```
```{r 5LHCtablepdf, eval=params$ispdf, cache=TRUE, dependson=c('5_table-functions','5_LHC-load')}
...
H.evolved <- exp.LHC$timing.H > 0
P.evolved <- exp.LHC$timing.P > 0
```
...
RF.coevo.H <-
  randomForest::randomForest(
    coevo.H ~ n.H + n.P + v.H + v.P + r.H + r.P + mU.HnP + mU.H1P + mU.PnH + mU.P1H + U.bP1 + U.bPn + U.bH1 + U.bHn + MaxArea,
    data = exp.LHC,
    mtry = 5,
    ntree = 1000,
    importance = T
...
RF.coevo.P <-
  randomForest::randomForest(
    coevo.P ~ n.H + n.P + v.H + v.P + r.H + r.P + mU.HnP + mU.H1P + mU.PnH + mU.P1H + U.bP1 + U.bPn + U.bH1 + U.bHn + MaxArea,
    data = exp.LHC,
    mtry = 5,
    ntree = 1000,
    importance = T
...
RF.depend.H <-
  randomForest::randomForest(
    depend.H ~ n.H + n.P + v.H + v.P + r.H + r.P + mU.HnP + mU.H1P + mU.PnH + mU.P1H + U.bP1 + U.bPn + U.bH1 + U.bHn + MaxArea,
    data = exp.LHC,
    mtry = 5,
    ntree = 1000,
    importance = T
...
RF.depend.P <-
  randomForest::randomForest(
    depend.P ~ n.H + n.P + v.H + v.P + r.H + r.P + mU.HnP + mU.H1P + mU.PnH + mU.P1H + U.bP1 + U.bPn + U.bH1 + U.bHn + MaxArea,
    data = exp.LHC,
    mtry = 5,
    ntree = 1000,
    importance = T
...
RF.timing.H <-
  randomForest::randomForest(
    timing.H ~ n.H + n.P + v.H + v.P + r.H + r.P + mU.HnP + mU.H1P + mU.PnH + mU.P1H + U.bP1 + U.bPn + U.bH1 + U.bHn + MaxArea,
    data = exp.LHC[H.evolved,], # filter out cases where humans did not evolved towards n
    mtry = 5,
    ntree = 1000,
    importance = T
  )
#acc.timing.H <- mean((predict(RF.timing.H, exp.LHC[H.evolved, 1:12]) - exp.LHC$timing.H) ^ 2)
...
RF.timing.P <-
  randomForest::randomForest(
    timing.P ~ n.H + n.P + v.H + v.P + r.H + r.P + mU.HnP + mU.H1P + mU.PnH + mU.P1H + U.bP1 + U.bPn + U.bH1 + U.bHn + MaxArea,
    data = exp.LHC[P.evolved,], # filter out cases where plants did not evolved towards n
    mtry = 5,
    ntree = 1000,
    importance = T
  )
#acc.timing.P <- mean((predict(RF.timing.P, exp.LHC[P.evolved, 1:12]) - exp.LHC$timing.P) ^ 2)
...
par(mar = c(5,1,3,1), cex.lab = plotScale * 0.5)
layout(matrix(c(1,2), ncol = 2))
...
```{r 5_find-number-trees, include=FALSE}
set.seed(777)

# prepare data
exp.LHC.r <- exp.LHC[sample(1:nrow(exp.LHC), nrow(exp.LHC)),]

exp.LHC.spl <- split(as.data.frame(exp.LHC.r), 
                 f = factor(ceiling(seq_along(1:nrow(exp.LHC.r)) / 1000)))

Grid <- expand.grid(c(2:6), c(500, 750, 1000))

# cluster and loop
cl <- makeCluster(6)
clusterExport(cl, list('randomForest', 'Grid', 'exp.LHC.spl'), envir = environment())

AUX <- parSapply(cl, 1:nrow(Grid), function(p) {
  aux <- c()
  for (i in 1:10) {
    X <- do.call(rbind, exp.LHC.spl[-i])
    RF.coevo.h <- randomForest(
      coevo.H ~ max.h + max.p + Um.ph + Um.hp + Ump.ph + Ump.hp + Kmp.h + Kmp.p,
      data = X,
      mtry = Grid[p, 1],
      ntree = Grid[p, 2]
    )
    RF.coevo.p <- randomForest(
      coevo.P ~ max.h + max.p + Um.ph + Um.hp + Ump.ph + Ump.hp + Kmp.h + Kmp.p,
      data = X,
      mtry = Grid[p, 1],
      ntree = Grid[p, 2]
    )
    acc.coevo.H <- 
      mean((predict(RF.coevo.h, exp.LHC.spl[[i]][, 1:8]) - exp.LHC.spl[[i]]$LM.h) ^ 2)
    acc.coevo.P <- 
      mean((predict(RF.coevo.p, exp.LHC.spl[[i]][, 1:8]) - exp.LHC.spl[[i]]$LM.p) ^ 2)
